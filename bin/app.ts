...
const readAsset = (filename: string) =>
  fs.readFileSync(`${assetsPath}/${filename}`, "utf8");
...
const trimLeadingWhitespace = (str: string) =>
  str.replace(/\n[ \t] +/g, "\n").trim();
...
for (const host of ["node-0", "node-1"]) {
  ...
  workerInstance.node.addDependency(assetsDeployment, serverInstance);
}

// configure jumpbox instance

const jumpboxUserData = UserData.custom(
  trimLeadingWhitespace(`
  #!/bin/bash
  set -xe
...
jumpboxInstance.node.addDependency(assetsDeployment, serverInstance);
